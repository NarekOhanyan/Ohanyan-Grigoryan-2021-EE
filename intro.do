...
* - saves files
*	/tex/sumstat.tex
*	/log/do_intro `c(current_date)'.smcl
*
* - saves figures
*	/figure/graph_`var'.`format'
*
* 	where
...
* Save logs
capture log close
log using "${path}/log/intro `c(current_date)'.smcl", replace
...
foreach var in FF_A PGDP_t1 PCPI_t1 PCPIX_t1 Output_gap_dt_t1 Output_gap_t1 VIX {
	...
	graph export "${path}/figure/graph_`var'.pdf", as(pdf) replace
	graph export "${path}/figure/graph_`var'.eps", as(eps) replace
}
...
est store sum_stat
esttab sum_stat using "${path}/tex/sumstat.tex", replace ///
mtitles("") ///
