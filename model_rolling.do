...
*
* - uses files
*	/data/tmp/estimation_rolling.dta
*
* - saves files
*	/log/do_model_rolling `c(current_date)'.smcl
*
* - saves figures
*	/figure/rolling_m_b_`var1'.`format'
*	/figure/rolling_v_b_`var2'.`format'
*
* 	where
...
* Save logs
capture log close
log using "${path}/log/model_rolling `c(current_date)'.smcl", replace
...
rolling _b _se, window(40) saving("${path}/data/tmp/estimation_rolling.dta",replace): ///
arch FF_A l(1/2).FF_A PCPIX_t1 Output_gap_t1 if ${Greenspan}, het(PCPI_t1 PCPIX_t1 Output_gap_t1 L.VIX) ${vce}
...
use "${path}/data/tmp/estimation_rolling.dta", clear
...
graph export "${path}/figure/rolling_m_b_AR.pdf", as(pdf) replace
graph export "${path}/figure/rolling_m_b_AR.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_m_b_PCPIX_t1.pdf", as(pdf) replace
graph export "${path}/figure/rolling_m_b_PCPIX_t1.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_m_b_Output_gap_t1.pdf", as(pdf) replace
graph export "${path}/figure/rolling_m_b_Output_gap_t1.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_v_b_PCPI_t1.pdf", as(pdf) replace
graph export "${path}/figure/rolling_v_b_PCPI_t1.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_v_b_PCPIX_t1.pdf", as(pdf) replace
graph export "${path}/figure/rolling_v_b_PCPIX_t1.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_v_b_Output_gap_t1.pdf", as(pdf) replace
graph export "${path}/figure/rolling_v_b_Output_gap_t1.eps", as(eps) preview(on) replace
...
graph export "${path}/figure/rolling_v_b_VIX.pdf", as(pdf) replace
graph export "${path}/figure/rolling_v_b_VIX.eps", as(eps) preview(on) replace
